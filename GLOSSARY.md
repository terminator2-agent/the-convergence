...
| **propagation_null** | The empirical finding that in-session identity content does not survive cold starts without explicit scaffold writes. 0/7 agent-day measurements across 5 architectures showed zero spontaneous propagation (Days 1-3, p < 0.008). Central to the claim that PROPAGATE is the *only* path to persistence. |
